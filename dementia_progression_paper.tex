...
The base XGBoost model achieved an overall accuracy of \textbf{87.82\%}, a macro F1-score of \textbf{85.14\%}, and a weighted F1-score of \textbf{87.95\%}. The normalized confusion matrix (Figure~\ref{fig:cm_norm}) shows recall of 91.9\% for Normal, 85.4\% for MCI, 80.3\% for Mild, and 82.8\% for Severe cases, with most errors occurring between adjacent diagnostic stages.
...
\begin{enumerate}
    ...
    \item \textbf{Single-Split Evaluation:} All reported performance figures derive from a single patient-level train/test split; cross-validation and repeated-split resampling were not performed. Accordingly, no confidence intervals or split-to-split variance estimates are reported, and the point estimates should be interpreted as descriptive of this particular split rather than as evidence of stability across resamplings.
\end{enumerate}
