...
\textbf{Results:} On an independent test set of 29,939 visits from 7,527 patients, the base XGBoost model achieved \textbf{87.82\%} accuracy, \textbf{85.14\%} macro F1, and \textbf{87.95\%} weighted F1. Class-specific F1-scores were 0.93 (Normal), 0.83 (MCI), 0.77 (Mild), and 0.88 (Severe). Of all misclassifications, 94.9\% were one stage off, with only 0.2\% three stages off. Performance was broadly consistent across sex, age, and race. SHAP analysis identified diagnosis trajectory, current diagnostic state, and age-diagnosis interaction features as the strongest predictors. Adding BorderlineSMOTE or ensembling reduced aggregate performance relative to the base model.

\vspace{0.5em}
\textbf{Conclusions:} Robust dementia risk stratification is achievable without expensive neuroimaging, using only routine clinical assessments. The framework offers a scalable, transparent, and demographically fair tool for clinical decision support in community healthcare and resource-constrained settings, supporting a ``Frugal AI'' paradigm that democratizes early Alzheimer's detection.
\end{abstract}
...
Recent advances in Artificial Intelligence (AI) and machine learning have yielded models capable of predicting AD progression with remarkable accuracy. However, many studies rely on multimodal data, including MRI, PET, or cerebrospinal fluid biomarkers. These approaches are costly or invasive, require specialist facilities concentrated in academic centers, and often use smaller datasets that increase overfitting risk; consequently, they are difficult to generalize and scale to population screening.
...
\begin{enumerate}
    ...
    \item \textbf{Genetic risk (10 features):} Five raw APOE/family-history variables and five engineered APOE4 indicators and age-risk features.
...
We evaluated \textbf{BorderlineSMOTE} \cite{han2005}---a variant of the Synthetic Minority Over-sampling Technique---only in the designated ablation configurations. When used, it was applied exclusively to the training subset after GroupShuffleSplit partitioning, ensuring that the test set contained only real, unaugmented patient visits. The final Config 1 model used no SMOTE.
...
We conducted a detailed analysis of the 3,646 misclassified samples (12.2\% error rate) to assess clinical safety (Table~\ref{tab:errors}).
...
\midrule
1 & \texttt{diagnosis\_mean} & \texttt{diagnosis\_mean} & \texttt{diagnosis\_mean} & target (current) \\
2 & target (current) & \texttt{diagnosis\_std} & \texttt{age\_diagnosis\_product} & \texttt{age\_diagnosis\_product} \\
3 & \texttt{age\_diagnosis\_product} & \texttt{is\_mci\_now} & \texttt{worst\_diagnosis\_ever} & \texttt{diagnosis\_mean} \\
4 & \texttt{diagnosis\_std} & \texttt{diagnosis\_changes} & \texttt{is\_mild\_now} & \texttt{diagnosis\_std} \\
5 & \texttt{NACCAGE} & \texttt{age\_diagnosis\_product} & \texttt{diagnosis\_std} & \texttt{diagnosis\_changes} \\
\bottomrule
\end{tabular}%
}
\end{center}
...
\begin{itemize}
    ...
    \item \textbf{Current Diagnostic State} (\texttt{target}, \texttt{is\_mci\_now}, and \texttt{is\_mild\_now}) provides strong baseline information for prognosis.
\end{itemize}
...
For clinical deployment, predicted probabilities must accurately reflect true outcome likelihoods. We evaluated calibration using reliability diagrams (Supplementary Figure S7). After correcting the probability-normalization step, the calibration ECE values are 0.127, 0.036, 0.099, and 0.046 for Normal, MCI, Mild, and Severe, respectively. Three of the four classes are well calibrated (ECE below 0.1); Normal remains moderately calibrated. Given the improvement already achieved relative to the initial model, we did not pursue further post-hoc calibration, as an additional retraining cycle for a marginal gain on one class would run counter to the simplicity the model is intended to demonstrate.
...
Prospective validation should first assess technical transportability across sites, temporal drift, calibration, subgroup performance, missing-data behavior, and referral thresholds before any patient-facing alert is enabled. A follow-on implementation study should measure workflow fit, alert burden, usability, adoption, time to specialist referral, clinician override patterns, equity of access, safety events, and changes in appropriate diagnostic work-up. It should also test whether model-supported referral improves clinically meaningful outcomes without increasing unnecessary testing. These staged evaluations would convert this retrospective proof of concept into a monitored clinical decision-support pathway in primary-care and community contexts.
